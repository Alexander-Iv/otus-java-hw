...
public class ReflectionHelper {
    ...
    public static void invoke(Object object, Method method) {
        logger.info(String.format("%-18s",  method.getName()));
        try {
            ...
            method.setAccessible(false);
            logger.info(String.format("%-18s, result = %s",  method.getName(), false));
            e.printStackTrace();
            throw new RuntimeException(e.getMessage());
        }
        logger.info(String.format("%-18s, result = %s",  method.getName(), true));
    }
